...
public class Merge {
    ...
    private static int[] forkInts(int source[]) {
        int sourceLen = source.length;
        if (sourceLen > 2) {
            int midIndex = sourceLen / 2;
            int result1[] = forkInts(Arrays.copyOf(source, midIndex));
            int result2[] = forkInts(Arrays.copyOfRange(source, midIndex, sourceLen));
            // 将两个有序的数组,合并成一个有序的数组
